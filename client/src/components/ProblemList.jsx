...
function ProblemList() {
  const [problems, setProblems] = useState([]); // State to store the fetched problems
  ...
  useEffect(() => {
    const fetchProblems = async () => {
      try {
        ...
        if (!response.ok) {
          // If the response is not OK, try to get a more specific error message from the server
  ...
  return (
    <div className="space-y-4">
      ...
      {problems.map((problem) => (
        <div key={problem._id} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
